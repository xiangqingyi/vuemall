<template>
    <div>
        ...
        <div class="container">
            <h4>填写核对订单信息</h4>
            <div class="row">
                <div class="col-md-4" v-for="item in resultList" :key="item.index">
                ...
                <div class="more" v-show="hasMore" @click="clickMore">MORE
                    <span ref="span"></span>
                </div>
            </div>
            <h5 class="title_order">支付方式</h5>
            <div class="payMethod">
                <SingleSelect :selectList="patMethodList" v-on:clickSelected="selectedPayMethod"></SingleSelect>
            </div>
                    ...
                    <table class="table" style="background: #f3fbfe">
                        ...
                        <tbody>
                            <tr v-for="item in orderList" :key="item.index">
                                <td><img class="cartImg" src="`static/products/${item.image}`" alt=""></td>
                                <td>{{item.name}}</td>
                                <td>{{item.sum}}</td>
                                <td>{{item.price}}</td>
            ...
            <div class="payBtn">
                ...
                <button class="btn btn-danger btn-lg" @click="sumbitOrder">提交订单</button>
            </div>
        </div>
        <Modal :modal="modal">
            <p slot="title">温馨提示</p>
                    ...
                    <div class="col-md-12">
                        ...
                        <form action="" class="form-horizontal">
                            <div class="form-group">
                                <label style="margin-top: 5px" for="" class="col-md-2 control-label">所在地区</label>
                                <div class="col-md-10">
                                    <v-distpicker :province="defaultPicker.province" :city="defaultPicker.city" :area="defaultPicker.area" @province="onChangeProvince" 
                                        @city="onChangeCity" @area="onChangeArea" @selected="onSelected">
                                    </v-distpicker>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group" style="margin-bottom: 0">
                                <div class="col-md-offset-2 col-md-10">
                                    <button class="btn btn-danger" @click="saveShippingAddress" :disabled="disabled">保存收货人信息</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            clickMore() {
                if (this.returnList.length === this.showLength) {
                    this.returnList = this.addressList;
                    this.$refs.span.style.transform = 'rotate(0deg)';
                } else {
                    const newList = [...this.returnList];
            ...
            del(id) {
                ...
                const userId = sessionStorage.getItem('id');
                if (!userId) return;
                axios.post('/vue-mall-api/shippingAddress/operate', {
                    id: id,
                    userId: userId,
                    operateId: 1
                }).then(res => {
            ...
            submitOrder() {
                ...
                let bodyData = Object.assign({}, {
                    ...
                    patMethod: this.selectPayMethod,
                    goodList: listArr,
