    ...
    <div>
                    ...
                    <div v-show="!showSearch" class="input-group">
                        ...
                        <span class="input-group-btn">
                            <button class="btn btn-danger" type="button">
                                <span @click="toSearch" class="glyphicon glyphicon-search" aria-hidden="true"></span>
                            </button>
                        </span>
                    </div>
                </div>
                <div class="col-md-4" :style="{'padding-right':0,'padding-top': !showCart ? '1.5%' : '4%'}">
                    <div class="login-bar" v-if="isLogin">
                        <div class="login-title">欢迎您
                            <router-link class="default-link" to="/user/center">
                               {{user.name}}
                            </router-link>
                        </div>
                        <DropDown :dropDown="dropDown" class="admin" style="margin: 0 10px">
                            <li slot="dropLi" v-for="item in menu" :key="item.index">
        ...
        <div class="bread-inner">
            <div class="container bread-inner-bar">
                <ol class="breadcrumb">
                    <li v-for="(item, $index) in navBread" :key="item.index">
        ...
        <Modal :modal="modal">
            <div slot="content" :id="id">
                ...
                <div class="form-group" ref="userEmail" style="display:none;">
                    ...
                    <input type="email" class="form-control" id="userEmail" v-model="user.email" v-verify.email="user.email" placeholder="请输入邮箱">
...
<script>
     ...
     export default {
         ...
         mounted() {
             axios.get('/vue-mall-app/users/session?t=' + Date.now()).then(res => {
         ...
         methods: {
             ...
             toLogin() {
                 ...
                 this.modal.title = '登录';
                 this.id = "login";
                 this.$refs.forget.style.visbility = "visible";
             ...
             toRegister() {
                 ...
                 this.modal.title = "注册";
                 this.id = "register";
                 this.$$refs.forget.style.visbility = "hidden";
             ...
             toLogout() {
                 axios.get('/vue-mall-api/user/logout?t=' + Date.now()).then(res =>{
                     if (res.data.code === 200) {
                         this.isLogin = false;
                         this.user.name = '';
                         this.$root.push('/');
